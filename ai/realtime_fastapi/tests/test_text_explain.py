# 터미널에 'poetry run python test_text_explain.py'를 실행 후,
# 긴 글 설명 프롬프트가 생성되며, HCX로부터 응답을 받을 수 있습니다. (.env 파일에 HCX_API_KEY 저장)
import sys
import os
...
focused_dataset_economy = [
    "경제 성장률",  # 단어
    "경제 성장률이 높아지면",  # 어구 (2~5단어)
    "경제 성장률이 높아지면 실업률이 감소하는 경향이 있다.",  # 문장 (6~20단어)
    """첫째, 인구 고령화로 인해 노동력 감소가 발생하고 있다.
    젊은 노동 인구가 줄어들면서 생산성이 저하될 위험이 있다.
...
focused_dataset_it = [
    ...
    [  # 문장 (6~20 단어)
        "비트코인에서 새로운 블록을 앞 블록에 연결하기 위해서는 해시를 만들어야 하고 이를 위해서는 암호를 풀어야 하는데, 암호를 풀기 위해서는 수학적으로 매우 복잡한 계산을 해야 한다.",
        "이 때문에 자본이 풍부한 참여자들은 더 많은 가상통화 보상을 받기 위해 소위 ‘채굴장’이라는 시설을 만들고, 엄청난 수의 고사양 컴퓨터를 계속 가동하여 가상통화를 채굴한다.",
    ...
    [  # 문단 (20 단어 이상)
        ...
        계산을 빨리 할 수 있는 성능 좋은 컴퓨터를 많이 가지고 있는 사람이 유리하다.""",
        """이 때문에 자본이 풍부한 참여자들은 더 많은 가상통화 보상을 받기 위해 소위 ‘채굴장’이라는 시설을 만들고,
        엄청난 수의 고사양 컴퓨터를 계속 가동하여 가상통화를 채굴한다. 문제는 여기에 엄청난 전력이 소모된다는 것이다.""",
        """전 세계적으로 비트코인 채굴에만 들어가는 전력이 아일랜드 전국민이 같은 기간 쓰는 전력과 맞먹는다는 통계도 있다.
        한 예로 2018년 초 러시아의 한 비트코인 채굴업체는 채굴에 필요한 전력을 충당하기 위해 발전소를 사들였다.""",
    ],
]
...
def test_hcx_api():
    ...
    """
    for dataset in focused_dataset_it:  # focused_dataset_economy, focused_dataset_it
        for focused in dataset:
            prompt = text_explain.generate_focused_prompt(
                text[1], focused
            )  # text[0], text[1]
