...
# 긴 글 퀴즈 문제 조회 테스트
def test_fetch_text_quiz(client, auth_headers, test_db):
    """
    ...
    # 정상 퀴즈 조회
    response = client.get(f"/text_quiz/{quiz_data.quiz_id}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["question"] == quiz_data.question
    assert data["options"] == quiz_data.options

    # 존재하지 않는 퀴즈 조회
    response = client.get("/text_quiz/9999", headers=auth_headers)
    assert response.status_code == 404
    assert response.json() == {"detail": "해당 퀴즈를 찾을 수 없습니다."}


# 긴 글 퀴즈 제출 테스트
def test_submit_text_quiz(client, auth_headers, test_db):
    """
    ...
    assert data["question"] == quiz_data.question
    assert data["options"] == quiz_data.options
    assert data["answer"] == quiz_data.answer
...
# 긴 글 퀴즈 풀이 조회 테스트
def test_fetch_text_quiz_solution(client, auth_headers, test_db):
    """
    ...
    # 최근 풀이 기록 조회
    response = client.get(f"/text_quiz/solve/{quiz_data.quiz_id}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["question"] == quiz_data.question
    assert data["options"] == quiz_data.options
    ...
    # 존재하지 않는 퀴즈 ID로 요청하는 경우
    response = client.get("/text_quiz/solve/9999", headers=auth_headers)
    assert response.status_code == 404
    assert response.json() == {"detail": "해당 퀴즈를 찾을 수 없습니다."}
    ...
    response = client.get(f"/text_quiz/solve/{new_quiz.quiz_id}", headers=auth_headers)
    assert response.status_code == 404
